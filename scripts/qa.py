...
def audit(path, themes):
    ...
    # --- medias ---
    hero = re.search(r'<div class="hero-bg"[^>]*><img src="([^"]+)"', s)
    if hero:
        r.check("/Stellantis/" in hero.group(1) or "/content/dam/" in hero.group(1),
                f"{name} : hero hors bibliotheque HD (/Stellantis/ ou DAM marque) -> risque de pixellisation")
